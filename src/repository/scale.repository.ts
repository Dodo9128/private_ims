...
import { CreateScaleDto, UpdateScaleDto } from "../web/scale/scale.dto";
...
export class ScaleRepository extends Repository<Scale> {
  ...
  async setScale(rows: Row, cellIndexMap: Map<string, number>, venueId: string, systemId: string, newOne: boolean) {
    ...
    let scaleInResource = +rows.getCell(cellIndexMap.get(Constant.COL_KEY_SCALE_SCALE_IN_RESOURCE) as number) as number;
